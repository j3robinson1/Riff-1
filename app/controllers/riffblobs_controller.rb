class RiffblobsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @riffblob.update(riffblob_params)
        format.html { redirect_to @riffblob, notice: 'Riffblob was successfully updated.' }
  ...
  def destroy
    @riffblob.destroy
    respond_to do |format|
      format.html { redirect_to riffblobs_url, notice: 'Riffblob was successfully destroyed.' }
  ...
  private
    ...
    def riffblob_params
      params.require(:riffblob).permit(:name, :scr, :_id)
    end
end
